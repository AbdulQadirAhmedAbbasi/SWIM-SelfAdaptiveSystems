...
SelectionStrategy *SelectionStrategy::create(const char *algName, cSimpleModule *module, bool selectOnInGate)
{
    ...
    ////////////////////////////////////////////////////////////////////////////////////
        ...
        strategy = new WeightedRoundRobinSelectionStrategy(module, selectOnInGate);
    }
...
WeightedRoundRobinSelectionStrategy::WeightedRoundRobinSelectionStrategy(cSimpleModule *module, bool selectOnInGate) :
    SelectionStrategy(module, selectOnInGate)
{
    lastIndex = -1;
    minimumWeight = 0;
...
int WeightedRoundRobinSelectionStrategy::select()
{
    // return the smallest selectable index
    for (int i = 0; i < gateSize; ++i) {
        lastIndex = (lastIndex+1) % gateSize;
        if (isSelectable(selectableGate(lastIndex)->getOwnerModule()) && (availableWeight > minimumWeight))
        {
            --availableWeight;
            return lastIndex;
        }
