...
/**
 * Uses Weighted Round Robin selection, but skips any module that is not available currently.
 */
class QUEUEING_API WeightedRoundRobinSelectionStrategy : public SelectionStrategy
{
    protected:
        int lastIndex; // the index of the module last time used
        int minimumWeight; // the minimum weight of the module
        int availableWeight; // the available weight of the module
    public:
        WeightedRoundRobinSelectionStrategy(cSimpleModule *module, bool selectOnInGate);
        virtual int select() override;
};
